...
class DeformConv2d(nn.Module):
    ...
    def forward(self, x): #x假装是一个pic
        offset = self.p_conv(x)
        if self.modulation:
            m = torch.sigmoid(self.m_conv(x))
 
        dtype = offset.data.type() #offset 18 = 2 x 3 x 3 ,原来的kernel为3 x 3
        ks = self.kernel_size
        N = offset.size(1) // 2 #需要采样的所有点数
        ...
        # 这个p就是卷积核中心点在输入特征图上的位置加上偏移量后的位置,表示卷积
        # (b, h, w, 2N)
        ...
        # (b, c, h, w, N)
        x_q_lt = self._get_x_q(x, q_lt, N)#left,right,top,bottom
        ...
        out = self.conv(x_offset)#这样跳着进行卷积即可
    ...
    def _get_p(self, offset, dtype):
        ...
        # (1, 2N, 1, 1)
        p_n = self._get_p_n(N, dtype) #这个是偏移
        # (1, 2N, h, w)
        p_0 = self._get_p_0(h, w, N, dtype) #这个是映射
        p = p_0 + p_n + offset
        return p
    ...
    def _reshape_x_offset(x_offset, ks):
        b, c, h, w, N = x_offset.size() #c是原来的3个通道,N指的是Kernel size
...
if __name__ == "__main__":
    ...
    x = Variable(torch.randn(1, 3, 256, 256)) #大小不变
